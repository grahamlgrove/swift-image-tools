...
$ReleaseBuild = Join-Path $ProjectDir "ReleaseBuild-$Version"
...
python -m PyInstaller --noconfirm --clean --windowed --name $AppName `
    ...
    --version-file (Join-Path $ProjectDir "version_info.txt") --distpath $ReleaseBuild --workpath "build-release-$Version" `
    --specpath "build-release-$Version" (Join-Path $ProjectDir "image_manipulator.py")
